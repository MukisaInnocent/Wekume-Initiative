            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            <div className="bg-white rounded-xl shadow-md p-6">
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between p-3 bg-secondary-50 rounded-lg"><div className="flex items-center gap-3"><CheckCircle className="text-secondary-600" size={20} /><span className="font-medium">Resolved</span></div><span className="text-2xl font-bold text-secondary-600">{analytics.supportForms.resolved}</span></div>
                                    <div className="flex items-center justify-between p-3 bg-accent-50 rounded-lg"><div className="flex items-center gap-3"><Clock className="text-accent-600" size={20} /><span className="font-medium">In Progress</span></div><span className="text-2xl font-bold text-accent-600">{analytics.supportForms.inProgress}</span></div>
                                    <div className="flex items-center justify-between p-3 bg-primary-50 rounded-lg"><div className="flex items-center gap-3"><FileText className="text-primary-600" size={20} /><span className="font-medium">New</span></div><span className="text-2xl font-bold text-primary-600">{analytics.supportForms.new}</span></div>
                                </div>
                            </div>
                            <div className="bg-white rounded-xl shadow-md p-6">
                                <h3 className="text-lg font-heading font-bold mb-4">Volunteer Applications</h3>
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between p-3 bg-secondary-50 rounded-lg"><div className="flex items-center gap-3"><CheckCircle className="text-secondary-600" size={20} /><span className="font-medium">Approved</span></div><span className="text-2xl font-bold text-secondary-600">{analytics.volunteerApplications.approved}</span></div>
                                    <div className="flex items-center justify-between p-3 bg-accent-50 rounded-lg"><div className="flex items-center gap-3"><Clock className="text-accent-600" size={20} /><span className="font-medium">Pending Review</span></div><span className="text-2xl font-bold text-accent-600">{analytics.volunteerApplications.pending}</span></div>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            {contentSections.map(section => (
                                <div key={section.id} className="bg-white p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow">
                                    <div className="flex justify-between items-start">
                                        ...
                                        <button onClick={() => openModal('edit_content', section)} className="p-2 text-primary-600 hover:bg-primary-50 rounded-lg"><Edit2 size={20} /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                {activeTab === 'partners' && (
                    <div>
                        ...
                        <div className="bg-white rounded-xl shadow-md overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-left"><thead className="bg-gray-50 text-gray-600 uppercase text-xs font-semibold"><tr><th className="px-6 py-4">Name</th><th className="px-6 py-4">Type</th><th className="px-6 py-4">Website</th><th className="px-6 py-4">Status</th><th className="px-6 py-4 text-right">Actions</th></tr></thead><tbody className="divide-y divide-gray-100">{partners.length === 0 ? (<tr><td colSpan="5" className="px-6 py-8 text-center text-gray-500">No partners found. Add your first partner!</td></tr>) : (partners.map((partner) => (<tr key={partner.id} className="hover:bg-gray-50"><td className="px-6 py-4 hover:text-primary-600 cursor-pointer" onClick={() => openModal('edit_partner', partner)}><div className="flex items-center gap-3">{partner.logo_url && <img src={partner.logo_url} alt={partner.name} className="h-8 w-8 object-contain rounded" />}<span className="font-medium text-gray-900">{partner.name}</span></div></td><td className="px-6 py-4 text-gray-600 capitalize">{partner.type}</td><td className="px-6 py-4 text-gray-600 text-sm">{partner.website ? <a href={partner.website} target="_blank" rel="noreferrer" className="text-primary-500 hover:underline">Visit Link</a> : '-'}</td><td className="px-6 py-4"><span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${partner.is_active ? 'bg-secondary-100 text-secondary-800' : 'bg-red-100 text-red-800'}`}>{partner.is_active ? 'Active' : 'Inactive'}</span></td><td className="px-6 py-4 text-right flex justify-end gap-3"><button onClick={() => openModal('edit_partner', partner)} className="text-primary-600 hover:text-primary-800" title="Edit"><Edit2 size={18} /></button>{user.role === 'super_admin' && <button onClick={() => handleDelete(adminAPI.deletePartner, partner.id, fetchPartners, 'partner')} className="text-red-600 hover:text-red-800" title="Delete"><Trash2 size={18} /></button>}</td></tr>)))}</tbody></table></div></div>
                    </div>
                )}

                {activeTab === 'reports' && (
                    <div>
                        <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-heading font-bold">Reports Management</h2><button onClick={() => openModal('create_report')} className="bg-primary-600 text-white px-6 py-2 rounded-lg hover:bg-primary-700 transition-colors flex items-center gap-2"><Plus size={20} /> Add Report</button></div>
                        <div className="bg-white rounded-xl shadow-md overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-left"><thead className="bg-gray-50 text-gray-600 uppercase text-xs font-semibold"><tr><th className="px-6 py-4">Title</th><th className="px-6 py-4">Year</th><th className="px-6 py-4">Downloads</th><th className="px-6 py-4">Status</th><th className="px-6 py-4 text-right">Actions</th></tr></thead><tbody className="divide-y divide-gray-100">{reports.length === 0 ? (<tr><td colSpan="5" className="px-6 py-8 text-center text-gray-500">No reports found. Upload your first report!</td></tr>) : (reports.map((report) => (<tr key={report.id} className="hover:bg-gray-50"><td className="px-6 py-4"><div className="font-medium text-gray-900">{report.title}</div></td><td className="px-6 py-4 text-gray-600">{report.year}</td><td className="px-6 py-4 text-gray-600">{report.downloads || 0}</td><td className="px-6 py-4"><span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${report.is_published ? 'bg-secondary-100 text-secondary-800' : 'bg-gray-100 text-gray-800'}`}>{report.is_published ? 'Published' : 'Draft'}</span></td><td className="px-6 py-4 text-right flex justify-end gap-3"><button onClick={() => openModal('edit_report', report)} className="text-primary-600 hover:text-primary-800" title="Edit"><Edit2 size={18} /></button>{user.role === 'super_admin' && <button onClick={() => handleDelete(adminAPI.deleteReport, report.id, fetchReports, 'report')} className="text-red-600 hover:text-red-800" title="Delete"><Trash2 size={18} /></button>}</td></tr>)))}</tbody></table></div></div>
                    </div>
                )}
